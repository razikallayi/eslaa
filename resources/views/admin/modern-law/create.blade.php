	...
	<div class="col-sm-12">
		<div class="card">
			<div class="header bg-project">
				<h2 class="">@if(isset($modernLaw)) {{"Edit"}} @else {{"Add"}}  @endif Modern Law </h2>
			</div>
			<div class="body">
				@if(isset($modernLaw))
				<form id="form_validation" method="POST" action="{{url('admin/modern-law/edit/'.$modernLaw->id)}}" enctype="multipart/form-data">
				{{method_field('PUT')}}
				@else
				<form id="form_validation" method="POST" action="{{url('admin/modern-law')}}" enctype="multipart/form-data">
				@endif
				{{csrf_field()}}
						...
						<div class="row clearfix">
							...
							<div class="col-sm-6">
								...
								<div class="form-group ">
									<div class="form-line">
										<input type="text" value="{{$modernLaw->title or old('title')}}" name="title" maxlength="191" required class="form-control" >
									</div>
								</div>
							</div>
							...
							<div class="col-sm-6">
								...
								<div class="form-group ">
									<div class="form-line">
										<input type="text" value="{{$modernLaw->title_ar or old('title_ar')}}" name="title_ar" maxlength="191" required class="form-control" >
									</div>
								</div>
							</div>
							...
							<div class="col-sm-6">
								...
								<div class="form-group">
									<div class="form-line">
										<textarea name="content" rows="10" class="form-control htmlEditor">{{$modernLaw->content or old('content')}}</textarea>
									</div>
								</div>
							</div>
							...
							<div class="col-sm-6">
								...
								<div class="form-group">
									<div class="form-line">
										<textarea name="content_ar" rows="10" class="form-control htmlEditor">{{$modernLaw->content_ar or old('content_ar')}}</textarea>
									</div>
								</div>
							</div>
							...
							<div class="col-sm-12">
								<div class="form-group">
									...
									<div id="result" class="img-preview preview-lg row">
										...
										<div id="image-preview-{{substr( old('image'),0,-4)}}" class="col-lg-3 col-md-4 col-sm-6 m-t-30" style="min-height:80px"><span><img class="img-responsive" src="{{url(App\Models\ModernLaw::IMAGE_LOCATION)."/". old('image')}}"></span></div>
										@endif
										@if(isset($modernLaw) && $modernLaw->image !=null )
										<div id="image-preview-{{substr($modernLaw->image,0,-4)}}" class="col-lg-3 col-md-4 col-sm-6 m-t-30" style="min-height:80px"><span><img class="img-responsive" src="{{url(App\Models\ModernLaw::IMAGE_LOCATION)."/".$modernLaw->image}}"></span></div>
										@endif
									</div>
								</div>
							</div>
...
<script>
	$(function() {
		...
		whyte.storageLocation= "{{App\Models\ModernLaw::IMAGE_LOCATION}}";

		whyte.postUrl = "{{url('admin/modern-law/image')}}";
		whyte.result = $('#result');
