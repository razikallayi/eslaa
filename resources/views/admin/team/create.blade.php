...
<div class="row">
	...
	<div class="col-sm-12">
		<div class="card">
			...
			<div class="body">
				@if(isset($team))
				<form id="form_validation" method="POST" action="{{url('admin/teams/edit/'.$team->id)}}" enctype="multipart/form-data">
				{{method_field('PUT')}}
				@else
				<form id="form_validation" method="POST" action="{{url('admin/teams')}}" enctype="multipart/form-data">
				@endif
				{{csrf_field()}}
							...
							<div class="col-sm-12">
								<div class="form-group">
									...
									<div id="result" class="img-preview preview-lg row">
										...
										<div id="image-preview-{{substr( old('image'),0,-4)}}" class="col-lg-3 col-md-4 col-sm-6 m-t-30" style="min-height:80px"><span><img class="img-responsive" src="{{url(App\Models\Team::IMAGE_LOCATION)."/". old('image')}}"></span></div>
										@endif
										@if(isset($team) && $team->image !=null )
										<div id="image-preview-{{substr($team->image,0,-4)}}" class="col-lg-3 col-md-4 col-sm-6 m-t-30" style="min-height:80px"><span><img class="img-responsive" src="{{url(App\Models\Team::IMAGE_LOCATION)."/".$team->image}}"></span></div>
										@endif
									</div>
								</div>
							</div>
...
<script>
	$(function() {
		...
		whyte.postUrl = "{{url('admin/teams/image')}}";
		whyte.result = $('#result');
