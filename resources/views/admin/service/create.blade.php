...
<div class="row">
	...
	<div class="col-sm-12">
		<div class="card">
			<div class="header bg-project">
				<h2 class="">@if(isset($service)) {{"Edit"}} @else {{"Add"}}  @endif Service </h2>
			</div>
			<div class="body">
				@if(isset($service))
				<form id="form_validation" method="POST" action="{{url('admin/services/edit/'.$service->id)}}" enctype="multipart/form-data">
					{{method_field('PUT')}}
					@else
					<form id="form_validation" method="POST" action="{{url('admin/services')}}" enctype="multipart/form-data">
						@endif
						{{csrf_field()}}
						...
						<div class="row clearfix">
							...
							<div class="col-sm-12">
								...
								<div class="form-group ">
									<div class="form-line">
										<input type="text" value="{{$service->title or old('title')}}" name="title" maxlength="191" required class="form-control" >
									</div>
								</div>
							</div>
							...
							<div class="col-sm-12">
								<div class="form-group">
									...
									<div id="result" class="img-preview preview-lg row">
										...
										<div id="image-preview-{{substr( old('image'),0,-4)}}" class="col-lg-3 col-md-4 col-sm-6 m-t-30" style="min-height:80px"><span><img class="img-responsive" src="{{url(App\Models\Service::IMAGE_LOCATION)."/". old('image')}}"></span></div>
										@endif
										@if(isset($service) && $service->image !=null )
										<div id="image-preview-{{substr($service->image,0,-4)}}" class="col-lg-3 col-md-4 col-sm-6 m-t-30" style="min-height:80px"><span><img class="img-responsive" src="{{url(App\Models\Service::IMAGE_LOCATION)."/".$service->image}}"></span></div>
										@endif
									</div>
								</div>
							</div>
	...
	<script>
		$(function() {
			whyte={};
			whyte.imageWidth = 70;
			whyte.imageHeight = 70;
			whyte.storageLocation= "{{App\Models\Service::IMAGE_LOCATION}}";

			whyte.postUrl = "{{url('admin/services/image')}}";
			whyte.result = $('#result');
